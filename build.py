...
def callback(args):
    ...
    grammer = workdir.parent / 'grammer'
    if not grammer.exists():
        grammer.mkdir()
    
    for dir in (Path.cwd() / 'src').iterdir():
        language = dir.name[dir.name.rindex('-') + 1: ]
        if (dir / 'queries').exists():
            if not (grammer / language).exists():
                shutil.copytree(dir / 'queries', grammer / language)
        elif (dir / dir.name / 'queries').exists():
            if not (grammer / language).exists():
                shutil.copytree(dir / dir.name / 'queries', grammer / language)
    ...
    package(str(grammer), str(workdir.parent / 'tree-sitter-{}.zip'.format('grammer')))
